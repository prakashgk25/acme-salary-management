...
public class EmployeeController {
        ...
        @GetMapping
        public Page<EmployeeResponse> search(@RequestParam(required = false) String search,
                        @RequestParam(required = false) String country,
                        @RequestParam(required = false) String department,
                        @RequestParam(required = false) String currency,
                        @PageableDefault(size = 25) Pageable pageable) {
                return s.search(search, country, department, currency, pageable);
        }
        ...
        @DeleteMapping("/{id}")
        public void delete(@PathVariable Long id, @RequestHeader("If-Match-Version") Long version) {
                s.delete(id, version);
        }
}
